...
void count_millis (timer_callback_args_t *p_args)
{
    ...
    {
        // If so, increment the millisecond value
        millis++;
    }
}
...
void hal_entry(void)
{
    ...
    volatile uint64_t ADCTimer=millis;
    volatile uint64_t displayToggleTimer = millis;
    volatile uint64_t hmiTimer = millis;
    ...
    while (millis < start_up_interval){
        continue;
    }
    ...
    while (true){
        ...
        if (((millis - ADCTimer) >= (adc_scan_interval - sensor_response_time)) && !sensors_on)
        {
            turn_sensors_on();
        }
        ...
        if (millis - displayToggleTimer > sensor_display_toggle_interval)
        {
        ...
        if (millis - hmiTimer > hmi_interval){ // check if HMI update interval has elapsed
            ...
            hmiTimer = millis; // update HMI timer
        }
