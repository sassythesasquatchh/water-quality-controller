...
/* Actual period: 0.00075 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x927c,
  .duty_cycle_counts = 0x493e, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = modbus_timer_cb,
...
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x186a0,
  .duty_cycle_counts = 0xc350, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = count_millis,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
{ .scan_mask = ADC_MASK_CHANNEL_17 | ADC_MASK_CHANNEL_20 | 0, .scan_mask_group_b = 0, .priority_group_a =
